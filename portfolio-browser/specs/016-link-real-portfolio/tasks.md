...
- [X] T013 Wire the Overview page-mount callback in `src/pages/overview.py`: fetch `client.list_accounts()` and `client.list_attributes()` into the two stores, populate `app-parameters-account`'s options (every account, per FR-001) and the metric-toggle panel's options + tooltips (every attribute, per FR-005/FR-006) (depends on T009, T011, T012). **Post-implementation bug fix (found via manual testing in Edge)**: originally triggered by `Input("app-parameters-bar", "children")` (T011's own Output), reasoned at the time to guarantee T011 resolves first. In practice this broke on *revisiting* Overview via the sidebar (Overview → Positions → Performance → Overview): the page got stuck on "Loading account performance…" with the Account/From/To controls unpopulated, because that trigger raced against Dash's own internal page-routing swap rather than reliably re-firing on every mount. Fixed by adding a `dcc.Interval(id="overview-mount-trigger", interval=200, max_intervals=1)` inside Overview's own layout and re-triggering this callback off `Input("overview-mount-trigger", "n_intervals")` instead — the standard, robust Dash idiom for "run this once per page mount," scoped entirely to this page's own component lifecycle with no cross-module signal to race against. The `pathname` guard was also dropped as unnecessary (the Interval only exists while Overview is mounted). Regression-covered by a new scenario in `overview_default_chart.feature` ("Returning to Overview after navigating away still shows the chart").
